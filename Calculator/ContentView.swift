...
//  Calculator
//
//  Created by Ahanaf Tahmid Rifat on 12/11/24.
//
...
struct ContentView: View
{
   ...
   // @State var showMathError = false
    ...
    var body: some View
    {
       ...
       /* .alert(isPresented: $showMathError)
        {
            Alert(
                title: Text("Mathe Error"),
                message: Text(visibleWorkings),
                dismissButton: .default(Text("Okay"))
            )
        } */
   ...
   /* func isDivisionByZero() -> Bool {
        if visibleWorkings.contains("/0") {
            return true
        }
        return false
    } */
    ...
    func validInput() -> Bool
    {
        ...
        // Check if any operator directly follows a minus operator
        for i in 1..<visibleWorkings.count {
            let currentChar = visibleWorkings[visibleWorkings.index(visibleWorkings.startIndex, offsetBy: i)]
            let previousChar = visibleWorkings[visibleWorkings.index(visibleWorkings.startIndex, offsetBy: i - 1)]
            
            // If minus is directly followed by any operator, return false
            if previousChar == "-" && operators.contains(String(currentChar)) {
                return false
            }
        }
